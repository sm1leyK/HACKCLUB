...
const DAILY_LOGIN_REWARD_AMOUNT = 30;
const DAILY_LOGIN_RULE_KEY = "daily_login_v1";
const DAILY_LOGIN_TRANSACTION_TYPE = "daily_login";
...
function getUtcWindow() {
  ...
  const claimDateUtc = todayStart.toISOString().slice(0, 10);
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (!["participant", "admin"].includes(profileQuery.data.role)) {
      return fail(403, "unsupported_profile_role", "Only human participant/admin profiles can claim daily login reward.");
    }

    const { wallet, walletCreated } = await getOrCreateWallet(admin, userId);
    const { todayStartUtc, tomorrowStartUtc, claimDateUtc } = getUtcWindow();

    const existingClaim = await admin
      .from("wallet_transactions")
      .select("id, amount, created_at")
      .eq("wallet_id", wallet.id)
      .eq("transaction_type", DAILY_LOGIN_TRANSACTION_TYPE)
      .eq("status", "posted")
    ...
    if (existingClaim.error) {
      return fail(500, "daily_login_check_failed", existingClaim.error.message);
    }
    ...
    const rewardCycleUpsert = await admin
      ...
      .upsert(
        {
          cycle_type: "daily_login",
          status: "completed",
          rule_key: DAILY_LOGIN_RULE_KEY,
          reward_amount: DAILY_LOGIN_REWARD_AMOUNT,
          max_winners: null,
          window_start: todayStartUtc,
          window_end: tomorrowStartUtc,
          processed_at: new Date().toISOString(),
          created_by: null,
          notes: "Daily login reward cycle for current UTC day.",
          metadata: {
            source: "claim-daily-login-reward",
            claim_date_utc: claimDateUtc,
          },
        },
        {
    ...
    const transactionInsert = await admin
      ...
      .insert({
        ...
        direction: "credit",
        transaction_type: DAILY_LOGIN_TRANSACTION_TYPE,
        status: "posted",
        amount: DAILY_LOGIN_REWARD_AMOUNT,
        balance_before: wallet.balance,
        balance_after: wallet.balance + DAILY_LOGIN_REWARD_AMOUNT,
        created_by: null,
    ...
    const walletUpdate = await admin
      .from("wallets")
      .update({
        balance: wallet.balance + DAILY_LOGIN_REWARD_AMOUNT,
        lifetime_earned: wallet.lifetime_earned + DAILY_LOGIN_REWARD_AMOUNT,
